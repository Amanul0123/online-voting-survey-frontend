...
function Profile() {
    ...
    useEffect(() => {
        ...
        // Fetch user details
        axios.get(`http://localhost:8091/api/user/voterId/${voterId}`)
            .then(response => {
                setUser(response.data);
                return axios.get(`http://localhost:8091/api/party/constituency-number/${response.data.constituencyNumber}`);
            })
            .then(response => {
    ...
    const handleVote = async (partyId) => {
        ...
        try {
            ...
            await axios.put(`http://localhost:8091/api/party/${partyId}/votes?votes=${newVoteCount}`);
            await axios.put(`http://localhost:8091/api/user/voterId/${user.voterId}/vote-status/true`);
